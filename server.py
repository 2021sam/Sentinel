...
# mqtt.subscribe_list = [ 'office/door1', 'office/door2', 'office', 'office/sonic', 'office/pir', 'office/lux', 'shop', 'zigbee2mqtt/Samsung', 'zigbee2mqtt/Aqara', 'flask', 'online', 'enforcer', 'enforcer/lux' ]
# MQTT Protocol → device client_name/sensor_type
mqtt.subscribe_list = [ 'Foyer/pir', 'Foyer/sonic', 'CourtYard/enforcer', 'OfficeDoors/door1', 'OfficeDoors/door2', 'OfficeLux/lux', 'Office', 'Shop', 'zigbee2mqtt/Samsung', 'zigbee2mqtt/Aqara', 'flask', 'online' ]
mqtt.mqtt_subscribe()
...
def alarmSchedule():
	...
	if start_time <= hm and end_time > hm:
		...
		if not alarm_set:
			...
			publish_command_mqtt( 'OfficeDoors/command/terminal', 'print=0,0,5,0,Alarm Set' )
...
def resetAlarm():
	...
	global sensors_status								#	11/13/2021  After reset set alarm
	...
	alarm_set = False
	sensors_status = [-1, -1, -1]
	publish_command_mqtt( 'OfficeDoors/command/terminal', 'print=0,0,4,0,Reset Alarm' )
...
def refresh_display():
	...
	if alarm_set:
		...
		publish_command_mqtt( 'OfficeDoors/command/terminal', 'print=0,0,4,0,Alarm Not Set' )
...
def add_recipients():
	...
	if d < 5:
		if h >= 1 and h < 10:
			recipients_add.append( keith_email )
			recipients_add.append( michael_email )
...
def notify(type, subject, body):
	...
	#	11.21.2021  Kept crashing after trying to notify how long no internet
	#	Post this to the log file if notify or if exception thrown.
...
def background_scheduler():
	...
	notify( 3, f'Sentinel Security - Started' , 'Sentinel Security System Started.  Check for down time.' )
...
def noInternet():
	...
	time.sleep( 5 )
	if lapse:
		notify( 3, f'Internet connectivity', f'The internet at {location} was down for {lapse} seconds.' )
...
def populate( door, status ):
	...
	if sensors_status[ door ] != status:		#	This can be eliminated ********************
		...
		if status:
			...
			log_event( subject )
			snapshot( 'officedoor', 6 )			#	1 → camera 1
		else:
...
def snapshot_thread( camera, snaps ):
	...
	if location == 'bear':
		for i in range( snaps ):
			if camera in ['officedoor', 'pir', 'lux']:
	...
	if location == 'blackhawk':
		...
		for i in range( snaps ):
			print( ip_camera[camera] )
			reoLink( ip_camera[camera] )
			time.sleep( 1 )
		#
		# if camera == 1:
		# 	if location == 'bear':
...
def reoLink( ip ):
	...
	while size < 100000:
		...
		# size = curl( 'http://10.0.0.64/cgi-bin/api.cgi?cmd=Snap&channel=0&rs=wuuPhkmUCeI9WG7C&user=admin&password=' )
		# size = curl( 'http://10.0.0.109/cgi-bin/api.cgi?cmd=Snap&channel=0&rs=wuuPhkmUCeI9WG7C&user=admin&password=' )
		size = curl( f'http://{ip}/cgi-bin/api.cgi?cmd=Snap&channel=0&rs=wuuPhkmUCeI9WG7C&user=admin&password=' )
		# print('Recurl')
		time.sleep( 1 )
		# curl( 'http://10.0.0.64/cgi-bin/api.cgi?cmd=Snap&channel=0&rs=wuuPhkmUCeI9WG7C&user=admin&password=' )
...
def trigger(type, message ):
	...
	if trigger_time + wait_time <= now:
		...
		if location == 'blackhawk':
			body = f'\n\nhttp://{ip_location_blackhawk}:{PORT}{p}'				#	Sent when trust use appropriately
...
def messages( topic, payload ):
	...
	if topic == 'zigbee2mqtt/Samsung':		# Change name to ManCave
		false = False
		true = True
		d = eval( payload )
		# print( d )
	...
	if topic == 'zigbee2mqtt/Aqara':	# Change name to Garage
		false = False
		true = True
		d = eval( payload )
	...
	if 'pir' in topic:
		if value > 0:
			...
			if lapse < 0 or lapse > 10:			# < 0 when 86400 mod 86400 = 0  at midnight
				...
				m = f'{ topic } = { value }'
				log_event( m )
				snapshot('pir', 2)										# I want it to state why it is taking a snapshot first.
				print( m )
	...
	if 'sonic' in topic:
		if value > 0:
			...
			if lapse < 0 or lapse > 10:			# < 0 when 86400 mod 86400 = 0  at midnight
				...
				m = f'{ topic } = { value }'
				log_event( m )
				snapshot('sonic', 2)										# I want it to state why it is taking a snapshot first.
				print( m )
	...
	if 'enforcer' in topic:
		if value > 0:
			...
			if lapse < 0 or lapse > 10:			# < 0 when 86400 mod 86400 = 0  at midnight
				...
				m = f'{ topic } = { value }'
				log_event( m )
				snapshot('CourtYard', 2)										# I want it to state why it is taking a snapshot first.
				print( m )
